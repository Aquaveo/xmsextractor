...
# conan install -o xmsextractor:pybind=True -s compiler.version=6 -s compiler.libcxx=libstdc++11 -if ./conan -g txt xmsextractor/${TRAVIS_TAG}@aquaveo/stable 
conan install -o xmsextractor:pybind=True -s compiler.version=6 -s compiler.libcxx=libstdc++11 -if ./conan -g txt xmsextractor/1.0.1@aquaveo/stable 
# get the path to the conan package
export PATH_TO_PYTHON_PACKAGE=$(cat ./conan/conanbuildinfo.txt | grep PYTHONPATH.*xmsextractor | sed -r 's/^PYTHONPATH=\["(.*?)"\]$/\1/')
